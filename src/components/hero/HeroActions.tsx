...
const HeroActions = () => {
  ...
  return (
    <nav className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start" aria-label="Primary CTA">
      <Link to={isFrench ? "/fr/pricing" : "/pricing"}>
        <Button 
          ...
          aria-label={isFrench ? "Voir les forfaits" : "View Pricing Plans"}
        >
          {isFrench ? "Voir les forfaits" : "View Pricing Plans"}
          <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" aria-hidden="true" />
        </Button>
      </Link>
      <Link to={isFrench ? "/fr/contact" : "/contact"}>
        <Button 
          variant="outline" 
          ...
          aria-label={isFrench ? "Contacter le Support" : "Contact Support"}
        >
