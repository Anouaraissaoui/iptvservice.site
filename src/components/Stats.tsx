...
const Stats = () => {
  ...
  const stats = [
    { number: "+50K", label: t('stats.customers'), icon: <Users className="w-8 h-8" /> },
    { number: "+40K", label: t('stats.channels'), icon: <Tv className="w-8 h-8" /> },
    { number: "+1,500", label: t('stats.sports'), icon: <Radio className="w-8 h-8" /> },
    { number: "+50K", label: t('stats.vod'), icon: <Film className="w-8 h-8" /> }
  ];
      ...
      <div className="container mx-auto px-4 relative">
        <div className="max-w-3xl mx-auto text-center mb-12 md:mb-20">
          <span className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 md:px-6 py-2 backdrop-blur-xl text-sm mb-6">
            ...
            <span className="text-white">{t('benefits.whyChoose')}</span>
          </span>
