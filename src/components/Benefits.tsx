...
const Benefits = () => {
  ...
  return (
    <section 
      className="py-8 sm:py-12 md:py-16 bg-navy-light relative overflow-hidden"
        ...
        <div className="grid lg:grid-cols-2 gap-6 md:gap-12 items-center">
          <figure className="relative group order-2 lg:order-1" role="img" aria-label="IPTV Service Features">
            ...
            >
              ...
              {isLoading && (
                <div className="absolute inset-0 bg-navy animate-pulse rounded-2xl" role="progressbar" aria-label="Loading image..." />
              )}
              <img 
                ...
                className={`relative rounded-2xl shadow-xl border border-white/10 group-hover:border-primary/50 transition-all duration-300 ${
              ...
              />
              {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-navy rounded-2xl" role="alert">
                  <p className="text-gray-400">Failed to load image</p>
                </div>
              )}
              <figcaption className="absolute -bottom-3 -right-3 bg-navy-light/90 border border-white/10 rounded-xl px-4 py-2 shadow-lg backdrop-blur-xl">
                <div className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full bg-primary animate-pulse" aria-hidden="true" />
                  <p className="text-white text-sm font-medium">Premium Quality Streaming</p>
                </div>
              </figcaption>
            </a>
          </figure>
          ...
          <div className="space-y-4 md:space-y-6 order-1 lg:order-2">
            <header className="space-y-3 text-center lg:text-left">
              <a 
                href="/features" 
                className="inline-flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-1.5 backdrop-blur-xl text-xs hover:bg-white/10 transition-all duration-300"
              ...
              >
                ...
                <span className="text-primary relative inline-block hover:text-primary/90 transition-colors">
                  IPTV Benefits
                  <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-primary/30 blur-sm" aria-hidden="true" />
                </span>
              </h2>
            </header>
            ...
            >
              {benefits.map((benefit, index) => (
                <li key={index}>
                  <a 
                    href={benefit.link}
                    className="block group bg-navy/50 border border-white/10 rounded-lg p-3 sm:p-4 hover:border-primary/50 transition-all duration-300 hover:bg-navy-light/50 hover:scale-[1.02] backdrop-blur-sm"
                  ...
                  >
                    <article className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="text-base sm:text-lg font-semibold text-white mb-1 group-hover:text-primary transition-colors">
                          {benefit.title}
                        </h3>
